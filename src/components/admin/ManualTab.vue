<template>
  <div class="py-8 px-4 sm:px-10">
    ...
    <div v-if="activeTab === 'overview'">
      ...
      <div class="rounded-xl mt-4" style="padding: 18px 22px; background: #f0f9ff; border: 1px solid #bae6fd;">
        <h3 class="text-base font-semibold" style="color: #0c4a6e;">데이터 백업</h3>
        <p class="text-base mt-2" style="color: #0369a1; line-height: 1.7;">
          이 프로그램은 학교 PC 한 대에만 데이터가 저장되므로, 그 PC에 문제가 생기면 백업 없이는 되살릴 방법이 없습니다.
          사이드바의 <strong>업데이트 & 백업</strong> 탭에서 백업 파일을 내려받을 수 있습니다.
          <strong>라운드를 마감(FINALIZED)한 직후</strong>처럼 중요한 시점마다 백업해 두는 것을 권장합니다.
        </p>
      </div>
    </div>
    ...
    <div v-else-if="activeTab === 'areas'">
      <p class="text-base mb-6" style="color: #475569; line-height: 1.7;">
        ...
        수치형 전형요소에는 기준값 단위(선택)를 입력하면 점수표 헤더와 입력 화면에 병기됩니다 (예: 시간, 등급). 직접 입력 전형요소의 단위는 항상 '점'으로 자동 표시됩니다.
        점수 기준을 등록한 뒤 전형요소 상세의 "점수 계산 테스트"에 값을 입력하면 실제 계산될 점수를 저장 없이 즉시 확인할 수 있습니다.
      </p>
      ...
      <div class="rounded-xl mb-5" style="background: white; box-shadow: 0 1px 4px rgba(0,0,0,0.07), 0 0 0 1px rgba(0,0,0,0.04); overflow: hidden;">
        <div class="px-6 py-4" style="border-bottom: 1px solid #f1f5f9;">
          ...
          <p class="text-base" style="color: #64748b; margin: 4px 0 0;">어떤 값에 몇 점을 줄지 정하는 기준표입니다. 수치 범위·범주 전형요소에만 사용하며, 직접 입력 전형요소에는 필요 없습니다.</p>
        </div>
        <div class="px-6 py-5 text-base" style="color: #475569; line-height: 1.7;">
...
const roundGuides = [
  ...
  },
  {
    ...
    items: [
      ...
      '등록된 지원을 클릭하면 상세 정보를 확인하고 수정하거나 취소할 수 있습니다. 수정 화면에서 대학·모집단위를 변경하는 것도 가능합니다.',
      '모든 입력이 완료되면 화면 오른쪽 상단의 "입력 완료 확정" 버튼을 클릭하여 완료를 알립니다. 확정 후 지원을 추가·삭제하면 확정이 자동으로 취소됩니다.',
      '관리자는 언제든지 지원 현황을 라운드 관리 탭에서 확인할 수 있습니다.',
      '졸업생은 관리자가 졸업생 학급 계정으로 로그인하여 지원자를 등록합니다.',
    ],
    warning: null,
  },
  {
    ...
    items: [
      ...
      '"종료하기" 버튼을 누르기 전, 학급별 입력 확정 현황이 표시됩니다. 미확정 학급이 있어도 종료는 가능하지만, 미확정 학급에서 입력이 완전히 끝났는지 반드시 담임교사와 확인하세요.',
      '기초 데이터가 누락된 지원자가 있으면 종료가 거부됩니다. 해당 데이터를 입력한 뒤 다시 종료해 주세요.',
    ],
    note: '✓ 점수 계산 중 오류가 발생하면 라운드가 자동으로 "진행중" 상태로 되돌아갑니다. 오류 내용을 확인하고 데이터를 수정한 뒤 다시 종료하면 됩니다. 종료 후에도 담임교사의 추가 입력이 필요하면 "다시 열기" 버튼으로 다시 진행중 상태로 전환할 수 있습니다.',
    warning: null,
  },
  {
    id: 'recommend',
    ...
    items: [
      '라운드 상세에서 대학·모집단위별 점수 순위를 확인합니다. 상단 토글로 "모집단위별 순위"와 "대학 전체 순위" 보기를 전환할 수 있습니다.',
      '결과 탭의 "자동 추천 확정" 버튼을 누르면 두 단계로 추천을 일괄 확정합니다. 1단계로 모집단위 순위순으로 모집단위 잔여 정원까지 채우고, 2단계로 대학 전체 정원이 있으면 모집단위별 순서를 유지한 채 대학 전체 순위로 인원을 줄입니다. 정원이 무제한이면 해당 단계의 컷이 걸리지 않습니다. 버튼을 다시 눌러도 이미 확정된 추천은 유지됩니다.',
      '2단계에서는 각 모집단위의 다음 차례 지원자끼리만 대학 전체 순위로 겨룹니다. 따라서 같은 모집단위 안에서 순위가 낮은 지원자가 높은 지원자를 앞질러 추천되는 일은 없습니다. 대학 전체 순위가 높아도 같은 모집단위의 상위 지원자에게 자리가 먼저 가면 이번 라운드에는 추천되지 않습니다.',
      '동점이 정원 경계를 가르는 경우(잔여석보다 동점 인원이 많을 때)에만 그 동점 그룹을 자동 확정하지 않고 "수동 확인 필요" 목록에 순위·잔여석·경합 인원과 함께 안내합니다. 그보다 상위인 학생은 그대로 자동 확정되며, 정원이 동점 그룹 사이에 정확히 떨어지면 수동 확인이 필요 없습니다.',
      '대학 그룹 머리글의 "이 대학 자동 추천" 버튼을 쓰면 그 대학만 같은 방식으로 처리할 수 있습니다.',
      '동점자가 정원을 초과하는 경우 관리자가 직접 선택해야 합니다. 시스템은 동점 중 일부만 고르지 않으므로, 학생별 "추천 확정" 버튼으로 직접 처리합니다(모집단위 정원·대학 전체 정원을 초과하면 거부됩니다).',
      '학생별 "추천 확정"도 모집단위 순위를 건너뛸 수 없습니다. 같은 모집단위에 아직 추천되지 않은 상위 순위 지원자가 있으면 거부됩니다. 단 동점자끼리는 서로 막지 않으며(관리자 선택 보장), 상위 지원자가 이미 추천됐거나 미선발 처리된 경우에도 막지 않습니다.',
      '결격·서류 미비 또는 정원에 들지 못한 지원자는 해당 학생의 행에서 "미선발 처리" 버튼을 누르고 사유를 입력하십시오. 라운드를 마감하려면 선발되지 않은 모든 지원자에 대해 미선발 처리가 완료되어야 합니다. 미선발 처리된 지원자는 자동 추천 후보와 수동 추천 순서 가드에서 모두 빠지므로 다음 순위 지원자를 바로 추천할 수 있습니다. "미선발 해제"로 언제든 되돌릴 수 있습니다. 포기(다음 항목 참고)와 달리 정원 집계에는 영향을 주지 않습니다.',
      '라운드가 종료(CLOSED)된 동안에는 순위의 기준이 되는 설정을 바꿀 수 없습니다 — 재학생 우선 설정(대학·모집단위)과 학생의 재학/졸업 구분이 그렇습니다. 저장된 순위가 종료 시점 기준으로 계산된 값이기 때문입니다. 바꾸려면 라운드를 다시 열고 고친 뒤 다시 종료하십시오(종료 시 순위가 재계산됩니다). 정원 변경은 종료 중에도 가능합니다.',
      '대학에 재학생 우선을 설정하면 그 대학 전체 순위에서 점수와 무관하게 재학생이 졸업생보다 먼저 순위가 매겨집니다. 전체 정원 내에서 재학생이 먼저 채워지고, 남은 자리에 졸업생이 배정됩니다.',
      '모집단위에만 재학생 우선을 설정하면 그 모집단위 안에서 재학생이 먼저 추천되며, 대학 전체 정원으로 인원을 줄일 때도 그 모집단위 내부 순서는 유지됩니다. 다른 모집단위 지원자와의 비교는 대학 설정을 따릅니다.',
    ],
    note: '✓ "미선발"과 "포기"는 다릅니다. 미선발 처리는 라운드가 종료(CLOSED)된 동안 관리자가 명시적으로 선발하지 않기로 결정하는 것으로, 사유를 반드시 기록하며 정원 집계에 영향을 주지 않습니다(결격·서류 미비·정원 미달 등 구체적 사유는 기록된 사유 텍스트로 구분합니다). 포기는 마감(FINALIZED) 후 추천된 학생이 진학을 철회하는 것으로, 다음 라운드에 자리가 반환되며 되돌릴 수 없습니다. 라운드를 다시 열면 확정된 추천이 전부 초기화되므로, 결격자 한 명 때문에 라운드를 다시 열지 말고 미선발 처리 기능을 사용하십시오.',
    warning: null,
  },
  {
    id: 'finalized',
    ...
    items: [
      '라운드를 마감하려면 모든 지원자에 대해 추천 확정 또는 미선발 처리가 끝나 있어야 합니다. 결정되지 않은 지원자가 한 명이라도 남아 있으면 마감할 수 없으며, 해당 명단이 표시됩니다. 정원 초과로 마감이 막히는 경우에도 오류 내용이 표시됩니다.',
